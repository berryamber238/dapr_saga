...
namespace Service.Genesis.Services;
...
public class GenesisService : IGenesisService
{
    ...
    private const string SERVICE_NAME = "Service.Genesis";

    public GenesisService(DaprClient daprClient, BusinessDataRepository businessRepo, CompensateLogRepository compensateRepo)
    {
    ...
    {
        Log.Information($"[Genesis] Processing transaction {transactionId}");
        try
        {
            ...
            await PublishEventAsync(transactionId, "Success", "Genesis processed");
            return true;
        }
        catch (Exception ex)
        {
            Log.Error(ex, $"[Genesis] Error processing {transactionId}");
    ...
    {
        Log.Information($"[Genesis] Compensating transaction {transactionId}");
        try
        {
            ...
            await PublishEventAsync(transactionId, "Compensated", "Genesis compensated");
            return true;
        }
        catch (Exception ex)
        {
            Log.Error(ex, $"[Genesis] Error compensating {transactionId}");
            return false;
        }
    }
